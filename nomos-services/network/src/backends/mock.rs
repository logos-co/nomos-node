...
pub enum MockBackendMessage {
    ...
    Broadcast {
        message: MockMessage,
        topic: Option<MockPubSubTopic>,
    },
    /// Subscribe to a particular Mock topic
    RelaySubscribe { topic: MockPubSubTopic },
    /// Unsubscribe from a particular Mock topic
    RelayUnsubscribe { topic: MockPubSubTopic },
    ...
    /// Send a message using Mock Light Push
    LightpushPublish {
        message: MockMessage,
        topic: Option<MockPubSubTopic>,
        peer_id: PeerId,
...
impl NetworkBackend for Mock {
    ...
    fn new(config: Self::Config) -> Self {
        ...
        waku_set_event_callback(move |sig| match sig.event() {
            Event::MockMessage(ref msg_event) => {
    ...
    async fn process(&self, msg: Self::Message) {
        match msg {
            MockBackendMessage::Broadcast { message, topic } => {
            ...
            MockBackendMessage::LightpushPublish {
                message,
                topic,
                peer_id,
