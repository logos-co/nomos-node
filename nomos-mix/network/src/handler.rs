...
use nomos_mix_queue::{coin_flipping::PermutedCoinFlippingQueue, Queue};

pub const PROTOCOL_NAME: StreamProtocol = StreamProtocol::new("/nomos/mix/0.1.0");
...
impl MixConnectionHandler {
    pub fn new() -> Self {
        ...
        Self {
            ...
            queue: Box::new(PermutedCoinFlippingQueue::new(1, NOISE.to_vec())),
    ...
    > {
        ...
        loop {
            match self.outbound_substream.take() {
                Some(OutboundSubstreamState::PendingOpenStream) => {
                    self.outbound_substream = Some(OutboundSubstreamState::PendingOpenStream);
                    return Poll::Pending;
                }
